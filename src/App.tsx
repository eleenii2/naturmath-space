...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (arShape === 'persegi' || arShape === 'segitiga' || arShape === 'lingkaran') {
      // Langsung lewati Welcome Screen dan masuk ke tahap 3 (Pintar Bangun Datar) untuk memuat penjelasan & 3D AR
      setPhase('ABSTRACT');
      setSelectedObject(arShape);
    }
  }, []);
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-gradient-to-b from-sky-400 via-emerald-100 to-amber-100 font-sans text-stone-800 select-none">
      ...
      <button
        onClick={toggleAudio}
        className={`fixed top-4 right-4 z-[100] flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-bold shadow-md cursor-pointer transition-all ${audioEnabled ? 'bg-emerald-600 text-white border-emerald-500 scale-105' : 'bg-white/90 text-stone-600 border-stone-300 hover:bg-white'
          }`}
