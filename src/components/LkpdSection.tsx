...
import lkpdPdf from '../assets/lkpd.pdf';

interface LkpdProps {
  onBack: () => void;
}

export function LkpdSection({ onBack }: LkpdProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6 bg-black/85 backdrop-blur-md font-sans animate-fade-in">
      <div className="bg-white rounded-[32px] w-full max-w-5xl h-[90vh] border-4 border-emerald-500 shadow-2xl flex flex-col overflow-hidden relative">
        ...
        <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white px-5 py-4 flex flex-wrap justify-between items-center gap-3 border-b-4 border-emerald-800 flex-shrink-0">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="font-black text-lg md:text-2xl tracking-wide uppercase drop-shadow-sm leading-tight flex items-center gap-2">
                <span>LEMBAR KERJA PESERTA DIDIK (LKPD)</span>
                <span className="bg-yellow-400 text-slate-950 text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider font-extrabold shadow-2xs">Fase B • SD</span>
              </h2>
              <p className="text-xs md:text-sm font-bold text-emerald-100">
                Eksplorasi Bangun Datar di Kebun NaturMath Space & Budaya Nusantara
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 ml-auto">
            <a
              ...
              title="Buka PDF di Tab Baru / Cetak"
            >
            ...
            <a
              href={lkpdPdf}
              download="LKPD - NaturMath Space.pdf"
              className="flex items-center gap-2 bg-white text-emerald-950 hover:bg-emerald-50 active:scale-95 px-4 py-2.5 rounded-xl font-black text-xs md:text-sm shadow-md transition cursor-pointer border border-emerald-200"
              title="Unduh Berkas LKPD PDF"
            >
              <Download size={16} className="text-emerald-600 animate-bounce flex-shrink-0" />
              <span>Download LKPD (PDF)</span>
            </a>
            <button
              onClick={onBack}
              className="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 active:scale-95 text-white px-4 py-2.5 rounded-xl font-black text-xs md:text-sm shadow-md transition cursor-pointer ml-1"
            >
              <ArrowLeft size={16} />
              <span>Kembali ke Kebun</span>
            </button>
          </div>
        </div>
            ...
            <div className="flex items-start md:items-center gap-3">
              ...
              <div>
                <h4 className="font-black text-slate-900 text-sm md:text-base flex items-center gap-2">
                  <span>Dokumen Resmi LKPD NaturMath Space</span>
                  <span className="bg-emerald-500 text-white text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">Siap Cetak (Printable)</span>
                </h4>
                <p className="text-xs text-slate-600 mt-0.5 leading-relaxed">
                  Berkas aktivitas siswa untuk dikerjakan secara mandiri atau berkelompok berdasarkan pengamatan objek bangun datar di Kebun Eksplorasi.
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 w-full min-h-[500px] bg-white rounded-3xl overflow-hidden border-2 border-slate-300 shadow-md relative flex flex-col">
            <div className="bg-slate-800 text-slate-200 px-4 py-2 text-xs font-bold flex justify-between items-center border-b border-slate-700 flex-shrink-0">
              <span className="flex items-center gap-2">
                ...
                Tampilan Pratinjau Dokumen PDF LKPD
              </span>
              <span className="text-slate-400">Gunakan tombol + / - pada viewer untuk memperbesar atau mencetak</span>
            </div>

            <iframe
              src={`${lkpdPdf}#toolbar=1&navpanes=1&view=FitH`}
              title="Berkas LKPD NaturMath Space PDF"
              className="w-full h-full flex-1 border-0 min-h-[500px]"
            >
              <div className="p-8 text-center space-y-4">
                <p className="text-slate-600 font-medium">Browser Anda tidak mendukung pratinjau PDF langsung di dalam halaman ini.</p>
                <a
                  href={lkpdPdf}
                  download="LKPD - NaturMath Space.pdf"
                  className="inline-flex items-center gap-2 bg-emerald-600 text-white font-bold px-5 py-2.5 rounded-xl shadow"
                >
                  <Download size={18} /> Download LKPD PDF
                </a>
              </div>
            </iframe>
          </div>
        ...
        <div className="bg-white px-6 py-3 border-t border-slate-200 flex flex-wrap justify-between items-center text-xs font-bold text-slate-400 flex-shrink-0 gap-2">
          <span className="flex items-center gap-1.5">
            ...
            Lembar Kerja Peserta Didik (LKPD) • Eksplorasi Geometri SD
          </span>
