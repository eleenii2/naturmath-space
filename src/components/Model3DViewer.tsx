...
export default function Model3DViewer({ assetName, isARMode = false }: Model3DViewerProps) {
  ...
  return (
    <div className={`w-full h-full relative ${isARMode ? 'bg-transparent' : 'bg-gradient-to-br from-slate-900 to-slate-950'} rounded-2xl overflow-hidden group ${isARMode ? '' : 'shadow-inner'}`}>
      {/* Control Overlay */}
      ...
      {/* R3F Canvas - Dioptimalkan untuk Mobile (DPR 1, tanpa shadow berat) & Desktop */}
      <Canvas 
        shadows={!isARMode && typeof window !== 'undefined' && window.innerWidth >= 768} 
        dpr={typeof window !== 'undefined' ? (window.innerWidth < 768 ? 1 : Math.min(window.devicePixelRatio, 1.5)) : 1}
        camera={{ position: [0, 0, 4], fov: 45 }}
        gl={{ powerPreference: 'high-performance', antialias: typeof window !== 'undefined' && window.innerWidth >= 768 }}
      >
        {!isARMode && <color attach="background" args={['#0f172a']} />}
        <ambientLight intensity={0.6} />
        <directionalLight position={[10, 10, 5]} intensity={1.2} castShadow={!isARMode && typeof window !== 'undefined' && window.innerWidth >= 768} />
