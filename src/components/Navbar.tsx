import { BookOpen, User, Info, ClipboardList, Camera } from 'lucide-react';
...
export default function Navbar({ currentPhase, setPhase, setActivePopup, onOpenARMenu }: NavbarProps) {
  return (
    <>
      ...
      <aside className="hidden md:flex fixed top-22 left-4 z-40 flex-col gap-2.5 pointer-events-auto font-kids select-none max-w-[195px]">
        ...
        {/* 6. Kamera AR Shortcut */}
        {onOpenARMenu && (
          <button
            onClick={onOpenARMenu}
            className="flex items-center gap-2.5 px-3 py-2.5 rounded-2xl shadow-xl border-2 border-rose-300 bg-gradient-to-r from-rose-600 via-pink-600 to-rose-700 text-white transition-all cursor-pointer hover:translate-x-1 active:scale-95 group text-left mt-1"
            title="Buka Kamera AR (Lihat AR)"
          >
            <div className="bg-white/20 p-1 rounded-lg shrink-0 animate-bounce" style={{ animationDuration: '2s' }}>
              <Camera size={18} className="text-white" />
            </div>
            <div className="leading-tight min-w-0">
              <span className="block text-[8px] font-bold uppercase tracking-wider text-rose-200">Kamera 3D</span>
              <span className="text-xs md:text-sm font-black truncate block text-amber-200 group-hover:text-white transition-colors">Lihat AR ✨</span>
            </div>
          </button>
        )}
      </aside>
      ...
      {(currentPhase === 'CONCRETE' || currentPhase === 'LKPD') && (
        <nav className="md:hidden fixed bottom-2 left-2 right-2 z-40 flex items-center justify-around gap-1 bg-slate-900/95 backdrop-blur-xl border-2 border-emerald-500/50 p-1.5 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] font-kids select-none pointer-events-auto animate-fade-in">
          ...
          {onOpenARMenu && (
            <button
              onClick={onOpenARMenu}
              className="flex flex-col items-center justify-center p-1 rounded-xl bg-gradient-to-r from-rose-600 to-pink-600 text-white font-black shadow-md hover:bg-rose-700 active:scale-95 transition-all flex-1 border border-rose-300/40"
            >
              <Camera size={16} className="animate-pulse" />
              <span className="text-[9px] font-black tracking-tight mt-0.5 text-amber-200">Lihat AR</span>
            </button>
          )}
        </nav>
      )}
