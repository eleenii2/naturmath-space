...
export default function DesaCanvas({ phase, onObjectSelect }: CanvasProps) {
  ...
  function onLoad(splineApp: any) {
    ...
    // 2.1. Gunakan fallback Three.js Raycaster dengan penyelarasan matriks dunia kamera
    const sceneObj = splineApp._scene || splineApp.scene;
    ...
    if (canvas && sceneObj && rawCamera) {
      ...
      // Raycaster Hover/MouseMove Handler
  ...
  return (
    <div className="w-full h-full relative bg-[#6fe7d4]">
      {/* HUD Info Teks Petunjuk Pembelajaran */}
      <div className="absolute top-24 left-1/2 transform -translate-x-1/2 z-30 text-center pointer-events-none w-11/12 max-w-xl">
        <div className="bg-slate-900/85 text-emerald-400 border-2 border-emerald-500/30 backdrop-blur-md px-6 py-2.5 rounded-full font-bold shadow-2xl text-xs md:text-sm tracking-wide font-sans">
          {hint}
        </div>
      </div>
      ...
      {loading && (
        <div className="absolute inset-0 bg-gradient-to-b from-sky-400 to-emerald-400 flex flex-col items-center justify-center z-40 space-y-4">
          <div className="w-14 h-14 border-4 border-white border-t-transparent rounded-full animate-spin"></div>
          <div className="text-center text-white font-bold tracking-wide">
            <p className="text-sm">Memanggil Desa Belajar Nusantara 3D...</p>
            <p className="text-[10px] opacity-70 mt-1">Dibuat menggunakan teknologi interaktif realistik</p>
          </div>
        </div>
      )}
      ...
      {/* Penutup / Blocker Logo Spline di Pojok Kanan Bawah (Diperluas maksimal untuk menutup total watermark Spline) */}
      <div className="absolute bottom-2 right-2 z-30 min-w-[220px] md:min-w-[260px] h-12 md:h-14 bg-white/95 backdrop-blur-xl px-5 py-2 rounded-2xl border-2 border-emerald-400 shadow-[0_4px_25px_rgba(16,185,129,0.3)] pointer-events-none flex items-center justify-center gap-3 select-none">
        <span className="text-xl md:text-2xl animate-bounce" style={{ animationDuration: '3s' }}>🌿</span>
        <div className="flex flex-col text-left leading-none">
          <span className="text-sm md:text-base font-black text-emerald-950 tracking-wider">NATURMATH SPACE 3D</span>
          <span className="text-[10px] md:text-xs font-extrabold text-amber-600 uppercase tracking-widest mt-0.5">Media Belajar Geometri</span>
        </div>
      </div>
    </div>
  );
