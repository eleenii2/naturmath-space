...
export default function LearningPanel({ phase, objectName, onNextPhase, onClose }: PanelProps) {
  ...
  // Buka otomatis AR Modal di mode 'EXPERIENCE' (Penjelasan Bangun Datar + Model 3D) jika diakses dari Google Lens (?ar=...)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const arShape = params.get('ar');
    if (arShape && (arShape === shape || arShape === objectName)) {
      setArModalPhase('EXPERIENCE');
      setShowARModal(true);
      // Bersihkan URL parameter agar tidak looping saat modal ditutup user
